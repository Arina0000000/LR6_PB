...
2. [Ход выполнения работы](#progress)
3. [Оформление отчёта](#create_report)
4. [Лог команд](#log)
...
Далее необходимо было установить Git с официального сайта, но данная система уже была установлена. Таким образом следующим шагом выполнения лабороторной работы стала настройка клиента git с указанием имени и адреса электронной почты при помощи команд `git config --global user.name` и `git config --global user.email`(рис. 3), позволяющих настроить глобальные параметры для всех проектов Git:
...
Затем был клонирован репозиторий с сервера на локальный компьютер с помощью команды `git clone` (рис. 4).
...
Так как нам необходимо производить редактирование веток и решение конфликтов остальные задачи будут производиться локально в среде **Git**. Для этого было проведено обновление рабочей копии репозитория, путём подтягивания последних изменеий из удалённого репозитория.

> [!IMPORTANT]
> Сначала необходимо перейти в рабочий репозиторий с помощью команды cd и только потом использовать команду `git pull` (рис. 7).
...
Для просмотра истории ветки branch1 (рис. 9) сначала необходимо перейти на данную ветку при помощи команды `git checkout`, после чего использовать команду `git log`. 
...
Следующим этапом необходимо было добавить измененные файлы при помощи команды `git add`. После чего, перед созданием коммита, был проверен статус репозитория при помощи команды `git status`. И наконец создан коммит, чтобы зафиксировать изменения. Все проведённые действия представлены на рисунке 15.
...
> [!IMPORTANT]
> При выполнении данной работы была неправильно написана команда удаления ветки из репозитория (`git push origin master --delete имя_удалённой_ветки`). Однако команда успешно удалила ветку branch1 и вызвала ошибку: удалённый репозиторий отказался удалить ветку master, так как она является текущей веткой.
> 
...
Получение истории операций в форматированном виде (сокращённый хэш + дата + имя автора + комментарий) представлено на рисунке 24.

<p align="center"><img src="https://github.com/Arina0000000/LR6_PB/blob/report/Screenshots_LR6/15.png"></p>
<p align="center">Рисунок 24 - Форматированный вывод истории операций</p></br>
...
Так для создания заголовков использовался символ # (количество символов # определяет уровень заголовка) (рис. 25). 

<p align="center"><img src="https://github.com/Arina0000000/LR6_PB/blob/report/Screenshots_LR6/headings.png"></p>
<p align="center">Рисунок 25 - Создание заголовков</p></br>

Для создания "жирного текста" необходимо выделить текст двойными звездочками или двойными подчеркиваниями (в данной работе использовались **) (рис. 26).

<p align="center"><img src="https://github.com/Arina0000000/LR6_PB/blob/report/Screenshots_LR6/big%20letter.png"></p>
<p align="center">Рисунок 26 - Создание жирного текста</p></br>

Пронумерованные списки были созданы при помощи чисел, за которыми следует точка, а маркированые списки были созданы при помощи символов `*, - или +`. Пример создания списков представлено на рисунке 27.

<p align="center"><img src="https://github.com/Arina0000000/LR6_PB/blob/report/Screenshots_LR6/lists.png"></p>
<p align="center">Рисунок 27 - Создание нумерованных и маркированных списков</p></br>

Также в данной работе была создана ссылка на изображение при помощи использования квадратных и круглых скобок (рис. 28). Изображения добавляются аналогично ссылкам с одним отличием: добавляется восклицательный знак перед квадратными скобками. 

В данной работе изображение добавляется иным способом.

<p align="center"><img src="https://github.com/Arina0000000/LR6_PB/blob/report/Screenshots_LR6/link.png"></p>
<p align="center">Рисунок 28 - Создание ссылок</p></br>

В языке разметки Markdown можно использовать HTML-элементы для добавления дополнительных стилей и элементов, недоступных в стандартном наборе Markdown. При выполнении лабораторной работы №6 использовался HTML-элемента `<img>`, который размещен внутри элемента `<p>`, выравнивающего изображение по центру с помощью атрибута `align="center"`, а атрибут `src` указывает URL изображения, хранящегося в отдельной папке в ветке report. 
...
Пример использования HTML-элементов для создания и выравния по центру изображений и подписей к ним представлено на рисунке 29.

<p align="center"><img src="https://github.com/Arina0000000/LR6_PB/blob/report/Screenshots_LR6/center.png"></p>
<p align="center">Рисунок 29 - Созданние HTML-элементов</p></br>

Для создания цитаты используется символ `>` (рис. 30), а для выделения кода используются обратные кавычки (`) (рис. 31).

<p align="center"><img src="https://github.com/Arina0000000/LR6_PB/blob/report/Screenshots_LR6/quote.png"></p>
<p align="center">Рисунок 30 - Созданние цитаты</p></br>

<p align="center"><img src="https://github.com/Arina0000000/LR6_PB/blob/report/Screenshots_LR6/code.png"></p>
<p align="center">Рисунок 31 - Выделение однострочного кода</p></br>
...
Для создания таких оповещений используется специальная строка blockquote, указывающая тип оповещения, а затем сведения об оповещении в стандартном блоквоте. Пример создания оповещания представлено на рисунке 32.

<p align="center"><img src="https://github.com/Arina0000000/LR6_PB/blob/report/Screenshots_LR6/warnings.png"></p>
<p align="center">Рисунок 32 - Созданние оповещения</p></br>
...
Изучение управления ветками, слияния и разрешения конфликтов помогло освоить работу с разными версиями кода. Создавались новые файлы, фиксировались коммиты и выполнялся их откат. Отчет был оформлен в формате Markdown с изображениями, которые размещенв в отдельной папке.

В процессе работы произошла ошибка при попытке удалить ветку: неправтльно была введена команда. Но команда выполнила частичное действие: branch1 была удалена, а master осталась нетронутой. В завершение изменения были успешно загружены на GitHub, что подтвердило усвоение материала. 
